...
public class SinkESMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //从配置文件中读取 es 的地址
        List<HttpHost> esAddresses = SinkES.getEsAddresses(parameterTool.get(ELASTICSEARCH_HOSTS));
        ...
        SinkES.addSink(esAddresses, bulkSize, sinkParallelism, data,
                (MetricEvent metric, RuntimeContext runtimeContext, RequestIndexer requestIndexer) -> {
                    requestIndexer.add(Requests.indexRequest()
                            .index(AKUN + "_" + metric.getName())  //es 索引名
                            .type(AKUN) //es type
